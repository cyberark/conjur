# frozen_string_literal: true

require_relative './commands/account/create'
require_relative './commands/account/delete'
require_relative './commands/db/migrate'
require_relative './commands/policy/load'
require_relative './commands/policy/watch'
require_relative './commands/role/reset_password'
require_relative './commands/role/retrieve_key'
require_relative './commands/server'
require_relative './commands/wait'
require_relative './commands/export'
require_relative './commands/configuration/show'
require_relative './commands/configuration/apply'
