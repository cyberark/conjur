class EdgeConfigurationController < RestController
  ...
  def log_message_get_role(id)
    "Validating role '#{id}'"
  end

  def get_role
    logger.debug{LogMessages::Endpoints::EndpointRequested.new(log_message_get_role(current_user.id))}
    allowed_params = %i[account]
    options = params.permit(*allowed_params).to_h.symbolize_keys
    begin
      render json: {
      ...
      logger.debug{LogMessages::Endpoints::EndpointFinishedSuccessfully.new(log_message_get_role(current_user.id))}
    rescue => e
      logger.error(LogMessages::Conjur::GeneralError.new(e.message))
      raise e
    end
