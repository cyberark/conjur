      ...
      module DataObjects
        ...
        class Utils
          class << self
            ...
            def includes_any?(array1, arrary2)
              (array1 & arrary2).first
            end
        ...
        class AuthenticatorContract < Dry::Validation::Contract
          ...
          rule(:claim_aliases) do
            if (duplicate = Utils.find_duplicate(Utils.alias_keys(values[:claim_aliases])))
              utils.failed_response(
                key: key,
          ...
          rule(:claim_aliases) do
            if (duplicate = Utils.find_duplicate(Utils.alias_values(values[:claim_aliases])))
              utils.failed_response(
                key: key,
          ...
          rule(:claim_aliases) do
            Utils.alias_keys(values[:claim_aliases]).flatten.each do |claim|
          ...
          rule(:claim_aliases) do
            bad_claim = Utils.includes_invalid_characters?(
              regex: 'a-zA-Z0-9\-_\.',
              items: Utils.alias_keys(values[:claim_aliases])
            )
          ...
          rule(:claim_aliases) do
            bad_claim = Utils.includes_invalid_characters?(
              regex: 'a-zA-Z0-9\/\-_\.',
              items: Utils.alias_values(values[:claim_aliases])
            )
            unless bad_claim.blank?
              utils.failed_response(
                key: key,
          ...
          rule(:claim_aliases) do
            denylist = %w[iss exp nbf iat jti aud]
            claim_keys_and_values = Utils.alias_keys(values[:claim_aliases]) + Utils.alias_values(values[:claim_aliases])
            if (bad_key = Utils.includes_any?(denylist, claim_keys_and_values))
              utils.failed_response(
                key: key,
          ...
          rule(:public_keys) do
            if values[:public_keys].present?
              begin
                json = JSON.parse(values[:public_keys])
                unless json.key?('value') && json['value'].is_a?(Hash) && json['value'].key?('keys') && json['value']['keys'].is_a?(Array) && json['value']['keys'].count.positive?
                  utils.failed_response(
                    key: key,
                    error: Errors::Authentication::AuthnJwt::InvalidPublicKeys.new(
