  ...
  module Util
    class OidcUtil
      ...
      def discovery_information
        ...
        Rails.cache.fetch(
          "#{@authenticator.account}/#{@authenticator.service_id}/#{URI::Parser.new.escape(provider_uri)}",
          expires_in: 5.minutes
        ) do
          @provider_discovery_config.discover!(provider_uri)
        rescue HTTPClient::ConnectTimeoutError, Errno::ETIMEDOUT => e
          raise Errors::Authentication::OAuth::ProviderDiscoveryTimeout.new(provider_uri, e.inspect)
        rescue => e
          raise Errors::Authentication::OAuth::ProviderDiscoveryFailed.new(provider_uri, e.inspect)
        end
      end
