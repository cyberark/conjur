  ...
  module DataHandlers
    class OngoingHandler
      ...
      def call(params, hostname, ip)
        ...
        allowed_params = [:edge_version, :edge_container_type, :edge_proxy, edge_statistics: [:last_synch_time, cycle_requests:
        ...
        # Log whether edge uses proxy
        edge_proxy = options['edge_proxy']
        edge_proxy_usage = ""
        if edge_proxy
          edge_proxy_usage = " using edge proxy"
        end
        ...
        @logger.info(LogMessages::Edge::EdgeTelemetry.new(tenant_id, edge.name, Time.at(last_synch_time_sec),
                                                         ...
                                                         edge.version, edge.platform, Time.at(installation_time_sec), edge_proxy_usage))
      end
