...
module EffectivePolicy
  ...
  class GetEffectivePolicy
    ...
    def initialize(
      ...
      @res_scopes = EffectivePolicy::Resources::ResourceScopes.new(@resource_repository,
                                                        role_id: role_id, account: account,
                                                        root_pol_id_pattern: root_pol_id_pattern,
                                                        absolute_depth: @absolute_depth)

      @user_scopes = EffectivePolicy::Resources::UserScopes.new(@resource_repository,
                                                     role_id: role_id, account: account,
                                                     root_pol_user_id_pattern: root_pol_user_id_pattern,
                                                     absolute_depth: @absolute_depth)

      @user_policy_resolver = EffectivePolicy::Resources::UserPolicyResolver.new(@absolute_depth, root_pol_user_id_pattern)
    end
