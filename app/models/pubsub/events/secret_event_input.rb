...
class SecretEventInput < EventInput
  ...
  INPUTS = {
    CREATE => {event_op: 'created', data: Struct.new(:specversion, :branch, :name, :owner, keyword_init: true)},
    DELETE => {event_op: 'deleted', data: Struct.new(:specversion, :branch, :name, keyword_init: true)},
    CHANGE => {event_op: 'value.changed', data: Struct.new(:specversion, :branch, :name, :version, keyword_init: true)}
  }
  ...
  def get_event_input(operation, db_obj)
    ...
    event_value = data.new(args).to_h.to_json
