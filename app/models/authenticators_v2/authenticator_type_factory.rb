...
module AuthenticatorsV2
  class AuthenticatorTypeFactory
    ...
    def create_authenticator_type(auth_dict)
      type = auth_dict[:type]
      if type.nil?
        return @failure.new(
    ...
    def create_authenticator_from_json(auth_json, account)
      ...
      auth_dict[:type] = type
      auth_dict[:service_id] = auth_dict[:name]
      auth_dict[:account] = account
      auth_dict[:variables] = auth_dict.delete(:data)

      format_owner(auth_dict)
      format_variables(auth_dict)
    ...
    def format_owner(auth_dict)
      owner = auth_dict[:owner]

      auth_dict[:owner_id] = "#{auth_dict[:account]}:#{owner[:kind]}:#{owner[:id]}" unless owner.nil?
    end
    ...
    # @param [Hash] identity - The identity variables
    # @return [Hash] - The processed identity variables
    def process_identity_variables(identity)
      processed_identity = {}

      identity.each do |identity_key, identity_value|
        if identity_key.to_s == "claim_aliases"
          claim_aliases = identity_value.to_h.map { |k, v| "#{k}:#{v}" }.join(",")
          processed_identity[identity_key] = claim_aliases
        elsif identity_key.to_s == "enforced_claims"
          enforced_claims = identity_value.join(",")
          processed_identity[identity_key] = enforced_claims
        else
          processed_identity[identity_key] = identity_value
        end
      end
