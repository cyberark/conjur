...
The security boundary (who can get an Azure access token) of the Azure Managed Identity Service is the resource to which it is attached to.
...
The security boundary of Conjur Azure authenticator is the access token received in the authentication request.
...
The full authentication process described in the [Authentication flow](#authentication-flow) section.
...
### Azure Access token
Conjur Azure authenticator receive an Azure access token which is a credential for the Azure resource. As such, Conjur handles this token with the appropriate sensitivity.

#### Azure Access token in transit
Any request to Conjur done over HTTPS to protect against any kind of Man-In-The-Middle attack. 

#### Azure access token at rest 
Conjur use that token only for the authentication flow and does not store it or log it into any persistent store.

#### Azure access token Time-To-Live (TTL)
Attack surface can be reduce by limiting Azure access token time-to-live (TTL).
The default TTL of an Azure access token is one hour, but it can reduce to shorter period. To reduce the TTL of Azure access token, please visit [Configurable token lifetimes in Azure Active Directory](https://docs.microsoft.com/en-us/azure/active-directory/develop/active-directory-configurable-token-lifetimes). 
###
...
| 2  | A host identity for Azure VM has not been added to privileged group                                                         | VM sends request with valid Azure token                             | <ul><li>Return 401 Unauthorized</li><li> Error message `RoleNotAuthorizedOnWebservice` is written to the log</li></ul>                         | Integration               | <ul><li> [ ] </li></ul>| No                                   |
...
| 4  | authn-azure isn't whitelisted in ENV or enabled in DB                                                                       | VM sends authentication request to Conjur with valid Azure token    | <ul><li>Return 401 Unauthorized</li><li>Error message `AuthenticatorNotEnabled` is written to the log</li></ul>                                | Integration               | <ul><li> [ ] </li></ul>| No                                   |
...
| 12 | `xms_mirid` field of the Azure token is missing                                                                             | VM sends authentication request to Conjur with invalid Azure token  | <ul><li>Return 401 Unauthorized</li><li>Error message `TokenFieldNotFoundOrEmpty` is written to the log</li></ul>                              | Integration               | <ul><li> [ ] </li></ul>| Yes                                  |
