...
| 8  | MissingRequestParam                                                   | Field '{0-field-name}' is missing or empty in request body                             | "Token" not in body of request                                                       | <ul><li> [ ] </li></ul> |
| 9  | TokenFieldNotFoundOrEmpty                                             | Field '{0-field-name}' not found or empty in token                                     |                                                                                      | <ul><li> [ ] </li></ul> |
| 10 | InvalidApplicationIdentity                                            | Application identity field '{0-field-name}' does not match Azure token                 | 1+ of the fields in the Azure token don't match the Conjur host identity annotations | <ul><li> [ ] </li></ul> |
| 11 | IdentityAssignedInvalid                                               | Two conflicting assigned identities were provided                                      | User inputs both user and system assigned identity in Conjur host annotations        | <ul><li> [ ] </li></ul> |
| 12 | ConcurrencyLimitReachedBeforeCacheInitialization                      | Concurrency limited cache reached before cache initialized                             |                                                                                      | <ul><li> [ ] </li></ul> |
| 13 | ConcurrencyLimitedCacheReached                                        | Concurrency limited cache reached the '{0-limit}' limit and will not call target       |                                                                                      | <ul><li> [ ] </li></ul> |

### Debug Log level

|    | Scenario                                                              | Log message                                                                            | Notes                                                                                | Implemented            |
|--- |-----------------------------------------------------------------------|----------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------|------------------------|
| 1  | ValidatingAnnotationsWithPrefix                                       | Validating annotations with prefix {0-prefix}                                          | Returned when validating if host annotations have proper prefix (authn-azure)        | <ul><li> [ ] </li></ul> |
| 2  | ValidatingHostId                                                      | Validating host id {0}                                                                 | Returned when VM attempts to authenticate, confirming resource has Conjur identity   | <ul><li> [ ] </li></ul> |
| 3  | ProviderUri                                                           | Working with Provider {0-provider-uri}                                                 | Returned when Conjur made a connection with provider uri and authenticating          | <ul><li> [ ] </li></ul> |
| 4  | ProviderDiscoverySuccess                                              | Provider discovery succeeded                                                           | Returned when Conjur made a connection with the provider-uri successfully            | <ul><li> [ ] </li></ul> |
| 5  | TokenDecodeSuccess                                                    | Token decode succeeded                                                                 | Returned during token decoding has been successful                                   | <ul><li> [ ] </li></ul> |
| 6  | TokenVerificationSuccess                                              | Token verification succeeded                                                           | Returned when provided `provider-uri` signature has been verified                    | <ul><li> [ ] </li></ul> |
| 7  | ClaimsFromToken                                                       | Extracting claims from token for resource {0-resource-name}                            | Returned when VM attempts to authenticate. Here, `xms_mirid` claims are extracted    | <ul><li> [ ] </li></ul> |
| 8  | ValidatingApplicationIdentity                                         | Resource identity for {0-resource-name} has been validated successfully                | Returned when `xms_mirid` claim matches Host identity                                | <ul><li> [ ] </li></ul> |     
| 9  | ValidatingRoleAnnotations                                             | Validating Role annotations for {0-role-name}                                          | Returned when host has proper combination of annotations for authn-azure             | <ul><li> [ ] </li></ul> |    
