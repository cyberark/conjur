...
describe "AWS Federation Token Dynamic secret input validation" do
  ...
  context "when creating aws federation token ephemeral secret with no issuer" do
    it "then the input validation passes" do
      ...
      params = ActionController::Parameters.new(name: "secret1", branch: "data/ephemerals", ttl: 120, method_params: method_params)
      expect { dynamic_secret.input_validation(params)
      }.to raise_error(Errors::Conjur::ParameterMissing)
    end
  end
...
describe "AWS Ephemeral secret annotations creation" do
  ...
  context "when creating aws ephemeral secret without inline policy" do
    it "all annotations are created" do
      method_params = ActionController::Parameters.new(region: "us-east-1")
      params = ActionController::Parameters.new(name: "secret1", branch: "data/ephemerals", ttl: 120, issuer: "issuer1", method:"federation-token", method_params: method_params)
      ...
      expect(get_field_value(annotations, "value", "name", Secrets::SecretTypes::DynamicSecretType::EPHEMERAL_METHOD)).to eq("federation-token")
  ...
  context "when creating aws ephemeral secret without method params" do
    it "all annotations are created" do
      params = ActionController::Parameters.new(name: "secret1", branch: "data/ephemerals", ttl: 120, issuer: "issuer1", method:"federation-token")
      annotations = dynamic_secret.convert_fields_to_annotations(params)
