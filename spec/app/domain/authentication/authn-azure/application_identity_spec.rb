...
RSpec.describe Authentication::AuthnAzure::ApplicationIdentity do
  ...
  before(:each) do
    define_host_annotation(host_annotation_type: user_assigned_identity_service_id_scoped_annotation, host_annotation_key: "#{granular_annotation_type}/user-assigned-identity", host_annotation_value: "some-user-assigned-service-id-scoped-value")
    define_host_annotation(host_annotation_type: system_assigned_identity_service_id_scoped_annotation, host_annotation_key: "#{granular_annotation_type}/system-assigned-identity", host_annotation_value: "some-system-assigned-service-id-scoped-value")

    define_host_annotation(host_annotation_type: non_azure_annotation, host_annotation_key: "#{global_annotation_type}/non-azure-annotation", host_annotation_value: "some-non-azure-value")
  end
  ...
  context "An application identity in annotations" do
    ...
    context("with a global scoped constraint") do
      ...
      it "Returns Hash of the constraint and its value" do
        expect(subject.constraints).to eq({ subscription_id: "some-subscription-id-value", resource_group: "some-resource-group-value", user_assigned_identity: "some-user-assigned-identity-value", system_assigned_identity: "some-system-assigned-identity-value" })
      end
    end
    ...
    context("with a service-id scoped constraint") do
      ...
      it "Returns Hash of the constraint and its value" do
        expect(subject.constraints).to eq({ subscription_id: "some-subscription-id-service-id-scoped-value", resource_group: "some-resource-group-service-id-scoped-value", user_assigned_identity: "some-user-assigned-service-id-scoped-value", system_assigned_identity: "some-system-assigned-service-id-scoped-value" })
      end
    end

    # Here we are only testing the scope of the subscription-id application identity because
    # we want to test that we grab the more granular application identity
