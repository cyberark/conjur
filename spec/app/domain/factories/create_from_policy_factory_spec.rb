      ...
      context 'when request is invalid' do
        context 'when request body is missing' do
          ...
          it 'returns a failure response' do
            ...
            expect(subject.message).to eq({ message: 'Request body must be JSON' })
        ...
        context 'when request body is malformed JSON' do
          ...
          it 'returns a failure response' do
            expect(subject.success?).to be(false)
            expect(subject.message).to eq({ message: 'Request body must be valid JSON' })
            expect(subject.status).to eq(:bad_request)
          end
        end
      ...
      context 'when role is not permitted to set variables' do
        ...
        context 'when role is not authorized' do
          it 'applies policy and variables' do
            ...
            expect(subject.success?).to be(false)
            expect(subject.message).to eq({ message: "Role is unauthorized to set variable: 'secrets/rspec/variable/foo%2Fbar%2Furl'" })
            expect(subject.status).to eq(:unauthorized)
          end
        end
        context 'when role lacks required privileges' do
          it 'applies policy and variables' do
            ...
            expect(subject.success?).to be(false)
            expect(subject.message).to eq({ message: "Role lacks the privilege to set variable: 'secrets/rspec/variable/foo%2Fbar%2Furl'" })
            expect(subject.status).to eq(:forbidden)
          end
        end
        context 'when variable is missing' do
          it 'fails with an appropriate error' do
            ...
            expect(subject.success?).to be(false)
            expect(subject.message).to eq({ message: "Failed to set variable: 'secrets/rspec/variable/foo%2Fbar%2Furl'. Status Code: '404', Response: ''" })
            expect(subject.status).to eq(:bad_request)
          end
        end
