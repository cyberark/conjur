# frozen_string_literal: true

require 'spec_helper'

describe ::CA::SSH::Certificate do
 
  describe '#render' do
    # Generated with `ssh-keygen -s [signing_private_key] -I [certificate_ID] [public_key_path]`
    let(:certificate_formatted) { "ssh-rsa-cert-v01@openssh.com " }
    let(:certificate) do
      Net::SSH::KeyFactory.load_data_public_key(certificate_formatted)
    end

    let(:formatted_certificate) { ::CA::SSH::Certificate.build(certificate).to_formatted }

    it "renders an SSH certificate to OpenSSH format" do
      expect(formatted_certificate.to_s).to eq(certificate_formatted)
    end

    it "returns a content type of 'application/x-openssh-file" do
      expect(formatted_certificate.content_type).to eq('application/x-openssh-file')
    end
  end
end
