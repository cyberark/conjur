...
class MockConjurEngineClient < ConjurDynamicEngineClient
  ...
  def normalize_hash_keys(hash, level = 0)
    super(hash, level)
  end
end
    ...
    double('net_http_post').tap do |net_http_post|
      if response_code
        allow(net_http_post)
          ...
          .and_return(
            MockHttpResponse.new(
              JSON.generate(mock_secret_error),
              response_code
            )
          )
      else
        allow(net_http_post)
          ...
          .and_return(
            MockHttpResponse.new(
              JSON.generate(mock_secret_result),
              201
            )
          )
      end
    ...
    it "then a dynamic secret is returned" do
      ...
      expect do
        MockConjurEngineClient.new(logger: logger, request_id: "abc", http_client: mock_ephemeral_secrets_service(nil))
                              .dynamic_secret(issuer_type, issuer_method, role_id, issuer_data, variable_data)
      end
        .to_not raise_error
