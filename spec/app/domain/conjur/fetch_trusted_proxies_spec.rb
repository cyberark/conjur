...
RSpec.describe 'Conjur::FetchTrustedProxies' do
  ...
  let(:valid_host) {"host/conjur/settings/trusted_proxies"}
  ...
  let(:mocked_role_class_list_duplications_3) {double("RoleClass")}
  let(:mocked_role_list_duplications_3) {double("RoleReturnsListWithDuplications")}
  let(:trusted_proxies_list_duplications_3) {[ip_1, ip_2, ip_3, ip_1, ip_2, ip_3]}
  let(:expected_list_3) {[trusted_proxies_list_duplications_3[0],
                          trusted_proxies_list_duplications_3[1],
                          trusted_proxies_list_duplications_3[2]]}
  ...
  before(:each) do
    ...
    allow(mocked_role_class_list_duplications_3).to receive(:by_login)
                                                        .with(valid_host, {:account => valid_account})
                                                        .and_return(mocked_role_list_duplications_3)
    allow(mocked_role_list_duplications_3).to receive(:restricted_to)
                                                  .and_return(trusted_proxies_list_duplications_3)
  ...
  context "A valid trusted proxies configuration" do
    context "trusted proxies contains an empty list of IP addresses" do
    ...
    context "trusted proxies contains a list of IP addresses without duplications" do
      subject do
        Conjur::FetchTrustedProxies.new(
    ...
    context "trusted proxies contains a list of IP addresses with duplications" do
      ...
      context "with 3 same IP addresses" do
        subject do
          Conjur::FetchTrustedProxies.new(
              role_cls: mocked_role_class_list_duplications_3,
              validate_account_exists: mocked_account_validator,
          ).call(
              account: valid_account
          )
        ...
        it "returns list with no duplication" do
          expect(subject).to eq(expected_list_3)
        end
      end
    end
  end
  ...
  context "An invalid trusted proxies configuration" do
    ...
    context "when role_cls.by_login returns nil" do
      ...
      context "then role_cls.roleid_from_username throws an error" do
        subject do
          Conjur::FetchTrustedProxies.new(
