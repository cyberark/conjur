...
describe Conjur::Certificates::CertUtils do
...
"""
    ...
    it 'parses a certificate' do
      expect(Conjur::Certificates::CertUtils.parse_certs(cert1_raw).map(&:to_der))\
        .to eq([cert1.to_der])
    end

    it 'parses two certificates' do
      expect(Conjur::Certificates::CertUtils.parse_certs(cert1_raw + cert2_raw).map(&:to_der))\
    ...
    it 'parses the certificate correctly even if the whitespace is wrong' do
      ...
      expect(Conjur::Certificates::CertUtils.parse_certs(bad_whitespace).map(&:to_der))\
        .to eq([cert1.to_der])
    end
    ...
    it 'shows a bad cert in error message' do
      ...
      expect do
        Conjur::Certificates::CertUtils.parse_certs(bad_cert)
...
"""
    ...
    context 'with one certificate in the chain' do
      subject{ Conjur::Certificates::CertUtils.add_chained_cert(store, one_certificate_chain) }
    ...
    context 'with two certificate in the chain' do
      subject{ Conjur::Certificates::CertUtils.add_chained_cert(store, two_certificates_chain) }
