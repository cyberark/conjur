...
RSpec.describe(Domain::Branch, type: :model) do
  let(:valid_name) { 'test-branch' }
  let(:valid_branch) { 'data' }
  let(:valid_owner) { instance_double(Domain::Owner, kind: 'user', id: 'alice', set?: true) }
  let(:valid_annotations) { { 'key1' => 'value1', 'key2' => 'value2' } }

  describe 'initialization' do
    it 'creates a valid branch with proper attributes' do
      branch = Domain::Branch.new(valid_name, valid_branch, valid_owner, valid_annotations)
  ...
  describe '.from_input' do
    ...
    it 'creates a branch from input hash' do
      ...
      allow(Domain::Annotations).to receive(:from_input).and_return(valid_annotations)

      branch = Domain::Branch.from_input(valid_input)
    ...
    it 'creates a branch with default owner when owner is empty' do
      ...
      allow(Domain::Owner).to receive(:new).and_return(valid_owner)
      allow(Domain::Annotations).to receive(:from_input).and_return(valid_annotations)

      branch = Domain::Branch.from_input(input_without_owner)
    ...
    it 'creates a branch with empty annotations when annotations are empty' do
      ...
      branch = Domain::Branch.from_input(input_without_annotations)
  ...
  describe '.from_model' do
    ...
    it 'creates a branch from model' do
      ...
      branch = Domain::Branch.from_model(model)
  ...
  describe '#identifier' do
    ...
    it 'returns correct identifier for root branch' do
      branch = Domain::Branch.new(valid_name, '/', valid_owner, valid_annotations)
