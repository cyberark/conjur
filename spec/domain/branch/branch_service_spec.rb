...
RSpec.describe(Domain::BranchService) do
  let(:owner_service) { instance_double(Domain::OwnerService) }
  let(:annotation_service) { instance_double(Domain::AnnotationService) }
  let(:res_service) { instance_double(Domain::ResourceService) }
  let(:res_scopes_service) { instance_double(Domain::ResourceScopesService) }
  ...
  let(:owner) { instance_double(Domain::Owner, kind: 'user', id: 'alice', set?: true) }
  let(:annotations) { { 'key1' => 'value1', 'key2' => 'value2' } }

  let(:branch) do
    instance_double(Domain::Branch,
                    name: 'branch1',
                    branch: 'data',
                    owner: owner,
  ...
  describe '#read_and_auth_branch' do
    it 'returns a branch when resource exists and user is authorized' do
      ...
      expect(Domain::Branch).to receive(:from_model)
        .with(policy)
        .and_return(branch)
  ...
  describe '#create_branch' do
    ...
    before do
      ...
      allow(Domain::Branch).to receive(:from_model).with(policy).and_return(branch)
    end
  ...
  describe '#update_branch' do
    let(:branch_up_part) do
      instance_double(Domain::BranchUpPart,
                      owner: owner,
                      annotations: annotations)
    end
  ...
  describe '#read_branches' do
    ...
    before do
      ...
      allow(Domain::Branch).to receive(:from_model).and_return(branch, branch)
    end
