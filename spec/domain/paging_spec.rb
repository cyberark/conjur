...
RSpec.describe(Paging) do
  describe '#initialize' do
    ...
    it 'raises error for negative offset' do
      expect { Paging.new(offset: -2) }.to raise_error(Domain::Validation::DomainValidationError)
    end

    it 'raises error for negative limit' do
      expect { Paging.new(limit: -1) }.to raise_error(Domain::Validation::DomainValidationError)
    end

    it 'raises error for limit above MAX_LIMIT' do
      expect { Paging.new(limit: Paging::MAX_LIMIT + 1) }.to raise_error(Domain::Validation::DomainValidationError)
    end
  end
