Feature: Users can authneticate with OIDC authenticator
  ...
  Scenario: Cache - Exceed refreshes_per_interval
    ...
    CONJ00018D
    """
    # Rate limited cache should be updated "refreshes_per_interval" times
    And The following appears "10" times in the log after my savepoint:
    """
    CONJ00016D
    """
  ...
  Scenario: Cache -
    ...
    # Update provider uri to an unreachable hostname
    When I save my place in the log file
    And I add the secret value "http://unreachable.com/" to the resource "cucumber:variable:conjur/authn-oidc/keycloak/provider-uri"
    And I authenticate "10" times in "10" threads via OIDC with id token
    # ID Token decoding should fail 2 times per call - one before refreshing certs and one after
    Then The following appears "20" times in the log after my savepoint:
    """
    CONJ00018D
    """
    # verify no request succeeded
    And The following appears "0" times in the log after my savepoint:
    """
    CONJ00017D
    """
    ...
    """
    When I save my place in the log file
    And I authenticate "100" times in "10" threads via OIDC with id token
    # We log that the limit has been reached per request
    Then The following appears "100" times in the log after my savepoint:
    """
    CONJ00020D
