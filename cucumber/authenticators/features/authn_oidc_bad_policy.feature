Feature: Users can authenticate with OIDC authenticator
  ...
  Scenario: id-token-user-property variable missing in policy is denied
    ...
    And I save the amount of log lines into "bookmark_variable_missing"
    When I authenticate via OIDC with id token
    Then it is unauthorized
    And The log filtered from line number in "bookmark_variable_missing" should contains "1" messages:
    """
  ...
  Scenario: webservice with read and no authenticate permission in policy is denied
    ...
    Given I get authorization code for username "alice" and password "alice"
    And I fetch an ID Token
    And I save the amount of log lines into "bookmark_no_authenticate_permission"
    When I authenticate via OIDC with id token
    Then it is unauthorized
    And The log filtered from line number in "bookmark_no_authenticate_permission" should contains "1" messages:
    """
