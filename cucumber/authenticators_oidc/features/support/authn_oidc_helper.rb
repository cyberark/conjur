...
module AuthnOidcHelper
  ...
  def authenticate_id_token_with_oidc(service_id:, account:, id_token: parsed_id_token, user_id: "")
    path = create_auth_url(service_id: service_id, account: account, user_id: user_id)
    payload = {}
    unless id_token.nil?
  ...
  def authenticate_code_with_oidc(service_id:, account:)
    path = "#{create_auth_url(service_id: service_id, account: account, user_id: nil)}?code=#{@url_oidc_code}&state=#{@url_oidc_state}"
    post(path, nil)
  end
  ...
  def oidc_claim_mapping
    @oidc_response_type ||= 'preferred_username'
  end
  ...
  def oidc_redirect_uri
    @oidc_redirect_uri ||= 'http://conjur:3000/authn-oidc/keycloak2/cucumber/authenticate'#create_auth_url(service_id: 'keycloak2', account: 'cucumber', user_id: nil)
  end
