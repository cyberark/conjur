...
When(/^I retrieve an API key for user "([^"]*)" using conjurctl$/) do |user_id|
  ...
  @conjurctl_stdout = File.read("~/admin-key.txt")
  puts "SIMPLECOV_DEBUG1-@conjurctl_stdout: #{@conjurctl_stdout}"
end

Then(/^the API key is correct$/) do
  #@conjurctl_stdout = File.read("~/admin-key.txt") #added ao
  adam_stdout = File.read("~/admin-key.txt") #added ao
  puts "SIMPLECOV_DEBUG2-@conjurctl_stdout: #{adam_stdout}"
