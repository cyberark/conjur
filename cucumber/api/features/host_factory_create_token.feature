...
Feature: Create a host factory token.
  ...
  Scenario: A host factory is invisible without some permission on it
    ...
    When I POST "/host_factory_tokens?host_factory=cucumber:host_factory:the-layer-factory&expiration=2050-12-31T00:00:00Z" with in-body params
  ...
  Scenario: Unauthorized users cannot create host factory tokens.
    ...
    When I POST "/host_factory_tokens?host_factory=cucumber:host_factory:the-layer-factory&expiration=2050-12-31T00:00:00Z"
  ...
  Scenario: A host factory token can be created by specifying an expiration time.
    ...
    When I successfully POST "/host_factory_tokens?host_factory=cucumber:host_factory:the-layer-factory&expiration=2050-12-31T00:00:00Z" with in-body params
  ...
  Scenario: A host factory token can be created by specifying an expiration time and CIDR.
    Given I permit user "alice" to "execute" it
    And I login as "alice"
    When I successfully POST "/host_factory_tokens?host_factory=cucumber:host_factory:the-layer-factory&expiration=2050-12-31T00:00:00Z&cidr[]=123.234.0.0/16&cidr[]=222.222.222.0/24" with in-body params
    Then our JSON should be:
    """
    [
      {
  ...
  Scenario: A host factory token cannot be created with invalid CIDR
    Given I permit user "alice" to "execute" it
    And I login as "alice"
    When I POST "/host_factory_tokens?host_factory=cucumber:host_factory:the-layer-factory&expiration=2050-12-31T00:00:00Z&cidr[]=123.234.0.0/16&cidr[]=1.895.abc.0/32" with in-body params
  ...
  Scenario: A host factory token cannot be created on resource which is not a host factory
    ...
    When I POST "/host_factory_tokens?host_factory=cucumber:group:test-group&expiration=2050-12-31T00:00:00Z" with in-body params
    Then the HTTP response status code is 422
    And there is an error
