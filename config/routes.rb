  ...
  scope format: false do
    ...
    constraints account: /[^\/\?]+/ do
      constraints authenticator: /authn-?[^\/]*/, id: /[^\/\?]+/ do
        ...
        #post '/authn-oidc(/:service_id)/:account/future-use/login' => 'authenticate#login_oidc'

        # authn-oidc has to be first as it can be ambgiuous with the optional :service_id & :id
        #post '/authn-oidc(/:service_id)/:account/future-use/authenticate' => 'authenticate#authenticate_oidc'
